# Este arquivo contém as rotas relacionadas aos leads, incluindo operações CRUD e listagem.
...
@jwt_required()
def listar_leads():
    # Rota para listar leads com paginação e filtro por status.
    try:
...
def obter_lead(lead_id):
    # Rota para obter detalhes de um lead específico pelo ID.
    try:
...
@jwt_required()
def criar_lead():
    # Rota para criar um novo lead com validação de campos obrigatórios.
    try:
...
def atualizar_lead(lead_id):
    # Rota para atualizar informações de um lead existente.
    try:
        lead = Lead.query.get(lead_id)

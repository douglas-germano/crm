...
def login():
    ...
    # Gerar tokens - embutindo o schema atual
...
def alterar_senha(usuario_atual):
    try:
        ...
        usuario_atual.set_senha(data['nova_senha'])
...
def esqueci_senha():
    ...
    # Valida workspace e troca search path
    tenant = Tenant.query.filter_by(subdominio=workspace).first()
    if not tenant:
    ...
    # Gera JWT / Timed Token Stateless
...
def redefinir_senha():
    ...
    # Aqui zera caso seja de primeiro acesso
